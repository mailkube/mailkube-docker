...
if command -v hadolint >/dev/null; then hadolint --config .hadolint.yaml Dockerfile || rc=1
...
step "shfmt"
if command -v shfmt >/dev/null; then shfmt -d -i 2 -ci -sr rootfs scripts || rc=1
...
step "Docker Hub README portability"
./scripts/check-readme-dockerhub.sh || rc=1
